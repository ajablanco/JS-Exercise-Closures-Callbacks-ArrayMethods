...
function processSum(numberList, callback) {
  const sum = numberList.reduce(function(accumulator, item) {
...
function processDuplicateFree(list, callback) {
  const listNew = list.filter((a,b) => list.indexOf(a) === b);
  return callback(listNew);
};
//yay
...
function firstNamesAllCaps(runners) {
  const newNames = runners.map(runner => runner.first_name.toUpperCase());
  return newNames;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const runnersShirt = runners.filter(runner => runner.shirt_size === tShirtSize);
  return runnersShirt;
}
